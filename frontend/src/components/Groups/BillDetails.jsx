...
const BillDetails = () => {
  ...
  const fetchBills = async () => {
    try {
      const res = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/getBills/${groupId}`);
      setBills(res.data || []);
    } catch (error) {
  ...
  const fetchGroupMembers = async () => {
    try {
      const res = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/getGroup/${groupId}`);
  ...
  const addBill = async () => {
    ...
    try {
      const res = await axios.post(`${process.env.REACT_APP_BACKEND_URL}/createBill`, {
        group: groupId,
  ...
  const deleteBill = async (billId) => {
    openConfirmModal("Are you sure you want to delete this bill?", async () => {
      try {
        let res = await axios.delete(`${process.env.REACT_APP_BACKEND_URL}/deleteBill/${billId}`);
        fetchBills();
        showToast(res.data.message, 'success');
        getBillBalances();
      } catch (error) {
  ...
  const updateBill = async () => {
    try {
      await axios.put(`${process.env.REACT_APP_BACKEND_URL}/updateBill/${currentBill._id}`, currentBill);
  ...
  const getBillBalances = async () => {
    try {
      ...
      const res = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/getBalances/${groupId}?userId=${user?._id}`);
